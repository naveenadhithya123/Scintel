...
export default function SuggesstionVerification() {
  ...
  // Data passed from Suggestions.jsx
  ...
  const handleSubmitOTP = async () => {
    ...
    try {
      ...
      if (verifyResult.verified) {
        // 2. Save Suggestion to DB
        // We MUST map the names exactly to what your backend controller expects
        const finalPayload = {
          ...
          // DB constraint: allowNull: false means we send a string even if empty
          proof_url: suggestionData.proof_url || "not-provided",
          // Backend constraint: user_id is required
          user_id: 1 // Defaulting to 1 as per controller requirements
        };
      ...
      {showOTP && (
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          <div className="relative w-[500px] h-[360px] bg-white rounded-2xl border border-[#CDE4EC] shadow-xl flex flex-col items-center justify-center px-10">
            ...
            <button disabled={loading} onClick={handleSubmitOTP} className="w-full h-[50px] bg-[#0B1C3D] text-white rounded-lg text-sm hover:bg-[#142d63] transition-all">
